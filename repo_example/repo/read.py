    ...
    def build_query(
        ...
        for filter_ in params.filters:
            if not hasattr(table_class, filter_.field):
                continue
                # Если требуется, чтобы все поля в фильтре соответствовали таблице
                # raise FieldNotFoundError(  # <- добавить класс
                #     f"Table {self.table_class.__tablename__} doesn't have field {filter_.field}"
                # )
